...
class News_m extends CI_Model {
	...
	{
		$this->db->select('hl_id,news_subtitle,focnews_id,news_wm,news_id,catnews_id,news_title,news_headline,news_title,news_caption,news_description,news_content,news_image_new,news_writer,news_datepub,tags_id,news_view,news_tags,news_id');
	...
	{
		$this->db->select('hl_id,news_subtitle,focnews_id,news_wm,news_id,catnews_id,news_title,news_headline,news_title,news_caption,news_description, news_content,news_image_new,news_writer,tags_id,news_datepub,news_view,news_tags,news_id');
        $this->db->from('db_news');
        $this->db->order_by('news_datepub','DESC');
		$this->db->limit($limit, $start);
		$this->db->where($kolom,$value);
		$this->db->where("news_datepub <",date('Y-m-d H:i',strtotime(date('Y-m-d H:i') . "+1 days")));
		$query = $this->db->get();
		return $query->result_array();
    }
